//===- aie.mlir ------------------------------------------------*- MLIR -*-===//
//
// Half-B vec_convexp_bf16_denorm capture design (GENERATED). Single compute tile (0,2) reads a
// 256-element batch from DDR via shim DMA, runs the vec_convexp_bf16_denorm kernel, writes the
...
module {
  aie.device(npu1_1col) {
    func.func private @convexp_bf16(memref<256xbf16>, memref<256xf32>)
        attributes {link_with = "convexp.o"}
    ...
    %core_0_2 = aie.core(%core) {
      ...
      func.call @convexp_bf16(%a, %o) : (memref<256xbf16>, memref<256xf32>) -> ()
    ...
    aie.runtime_sequence @sequence(%a: memref<256xbf16>, %c: memref<256xf32>) {
      aiex.npu.dma_memcpy_nd(%a[0, 0, 0, 0][1, 1, 1, 256][0, 0, 0, 1]) {id = 0 : i64, metadata = @inA} : memref<256xbf16>
      aiex.npu.dma_memcpy_nd(%c[0, 0, 0, 0][1, 1, 1, 256][0, 0, 0, 1]) {id = 1 : i64, metadata = @outC} : memref<256xf32>
