...
// vector_kernel_specs.py, not this file). Convert (round-narrow), f32 -> bf16 through an accfloat accumulator, normal-finite inputs. Mode-sweep point: rnd=3(symmetric_ceil).
//
//===----------------------------------------------------------------------===//
...
void conv_bf16(float *restrict in, bfloat16 *restrict out) {
  event0();
  ::aie::set_rounding(aie::rounding_mode::symmetric_ceil);
