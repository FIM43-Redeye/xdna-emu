...
### iter27 EVENT-SOURCE FORK, HALF-SETTLED (2026-07-10, post-compact): the 0x2450 wall is FAITHFUL firmware doing an internal run-fn dispatch on a garbage a7 -- NOT a genuine MMIO completion. Framing verified. Reframes the deeper gate toward the Zephyr IDLE thread / no-ready-task handling.

Maya's fork for this phase: is the event `deliver_pending_events` waits on a **Zephyr sync object** (internal -> fidelity fix) or a **genuine MMIO completion** (external -> synthesize)? Pinned the 0x2450 wall to the instruction level with `m2c_probe_trace_to_wall` + `m2c_probe_addr_store_watch` + framing check. Result decisively favors INTERNAL.

**1. `[0x12064]=0x2450` is INIT's saved a7, written by the firmware's OWN context-save -- not external, not MMIO, not leaked.** `m2c_probe_addr_store_watch` on `[0x12048..0x12068]` shows the block written ONCE, at n=47470-47489, all by `FUN_00002730` (the same fn that later resumes it). It is a register-context SAVE frame: `[+0]=a0=0xa0003dfc` (windowed return addr, call bits 0b10), `[+4]=a1=0x12120` (SP), `[+0x18]=a6=0x10f10` (INIT's own task ptr), **`[+0x1c]=a7=0x2450`**, etc.

**2. The resume is `restore a4..a15; Callx8 a7` -- MERT run-to-completion dispatch (call the task's run-fn = [frame+0x1c]).** `m2c_probe_trace_to_wall` tail: n=49452-49463 `FUN_00002730+0x32d..+0x343` does `L32iN a4..a15,[a3+0x10..0x3c]` with a3=0x12048, so **a7 <- 0x2450** (n=49456). Then n=49471 `Call0 0xdf98` lands directly on the bare `Callx8 a7` in trampoline `FUN_0000df8c`, n=49472 `Callx8 a7` with a7=0x2450 -> n=49473 fetch at 0x2450=zeros=wall.

**3. Framing VERIFIED faithful (both EMU-divergence smells refuted).** Added `XDNA_FW_DISASM_OVL=lo:hi` to `m2c_probe_disasm_range` (registers a +0x100 fetch overlay so any region can be compared base-vs-relocated). Base framing of `0xdf50..0xdfa4` is a clean coherent trampoline (`mov a11,a3; l32i a7,[a7+12]; ...; beqz a7; ...; callx8 a7; retw`); the +0x100 framing is GARBAGE (Unknowns at 0xdf7f/0xdf91/0xdf97/0xdf9a, mid-instr desync). So `Callx8 a7` at 0xdf98 is REAL, not a mis-decode. And the `Call0` skipping the `BeqzN a7` null-guard is moot: a7=0x2450 is non-null, so the guard would not catch it either way. The `Callx8 a7=0x2450` is exactly what silicon's firmware does with INIT's garbage run-fn slot.

**NET on the fork.** The wall is NOT a genuine MMIO completion and NOT external stimulus -- it is the firmware faithfully executing its own scheduler resume path on a task (INIT) whose run-fn slot is garbage because INIT was adopted mid-execution and no real task was ever created/readied to dispatch instead. Converges with the ISR-observation fact (the ISR reads ZERO MMIO; completion is a memory flag) and iter26's top-down chain. **Evidence now points AWAY from "external MMIO completion" and TOWARD an internal fidelity issue.**

**NEW REFRAME (strong, testable next).** In run-to-completion + Zephyr, when the scheduler finds NO ready task it must dispatch the **Zephyr IDLE thread** (`waiti`, our `FUN_0000c8e0`), not resume INIT's frame with a garbage a7. So the 0x2450 wall may be the "no ready task" path itself diverging: the idle thread should be selected and boot should settle at its `waiti` -- which IS the boot-to-idle goal. Two internal suspects, both Zephyr-source-checkable, NEITHER external MMIO:
1. **Idle-thread / no-ready-task handling.** Is the Zephyr idle thread registered in MERT's runnable/ready set, and is the scheduler's empty-ready path supposed to select it? If it exists but isn't picked (or was never registered because an init step didn't faithfully run), that is the divergence, and the correct terminal state is idle `waiti`. CHECK: is `FUN_0000c8e0`/`waiti` ever reachable; is the idle task in the runnable array `[0x2288..]`.
...
### iter31 EPC-advance VERIFIED faithful; the ONLY broken thing is the hook ptr a7=0x2450 (2026-07-10, autonomous)

Pulled the rest of the Zephyr Xtensa syscall path and ran two diagnostics; narrowed the wall to a single garbage value with everything around it proven faithful.

**Zephyr reference (syscall EPC advance).** `arch/xtensa/core/userspace.S` `xtensa_do_syscall`: `rsr.epc1 a2; addi a2,a2,3; wsr.epc1 a2` (line 33) and stores `EPC1+3` as the resume PC (`addi a3,a3,3; s32i a3,...pc_OFFSET`). SYSCALL is 3 bytes; the handler MUST advance past it or the rfe re-executes SYSCALL forever.

**Our MERT handler does this correctly.** `m2c_probe_addr_store_watch` on the frame PC block: at n=47495 `FUN_00002730+0x21b` stores `[0x12098] (frame+80) <- 0x08b043e4`. The Syscall is at `0x8b043e1`; `+3 = 0x8b043e4` (the wrapper's `RetwN`). So the saved resume-PC is EPC1+3, faithfully advanced -- the resume would `rfe` to `0x8b043e4 -> RetwN -> init 0x3dfc`. NOT the bug. (Also saved: frame+72=0x60032, frame+76=0, frame+84=0 -- SAR/loop context.)

**The resume tail is entirely faithful; only the hook ptr is garbage.** Full resume = `Callx8 a7` (on-resume hook, a7=frame+0x1c=0x2450) -> restore SAR/EPC1(=0x8b043e4)/loops/URs -> `rfe`. The EPC1, the continuation, the advance -- all correct. The sole defect is the hook pointer `a7 = 0x2450`.

**Skip-the-hook diagnostic (`m2c_probe_skip_resume_hook`, new).** Simulated a null-guarded callback: at the resume Call0 (0x2a86) jump to the post-callback site (0x2a89) with a2=0, bypassing the crash. Result: the 0x2450 crash is AVOIDED (confirming the hook is the crash site), but boot does NOT reach init's continuation 0x3dfc -- it spins in `FUN_00002730`'s save region (final pc 0x2974) after a single skip. INCONCLUSIVE: the crude skip leaves the restore path in a state that re-enters the handler rather than cleanly rfe-ing. So the hook is not trivially skippable; either it does essential work or a faithful hook value is required. (Probe kept for follow-up; self-skips without XDNA_FW_PROBE.)

**STATE: the wall reduces to a single question -- is `frame+0x1c = a7 = 0x2450` faithful or a divergence?** Everything else (EPC advance, resume-PC, restore, rfe target, Call0 decode, window-spill model, framing) is verified faithful. The hook is a per-task on-resume callback = the task's saved a7; for init it is task_create's leftover create-registry ptr 0x2450. Because init self-enqueues (cooperative) yet resuming any task via `Callx8 [arbitrary saved a7]` is nonsensical unless the saved a7 is a deliberate resume-fn, the ABI contract of `frame+0x1c` is the crux and is not recoverable from the stripped image.

**RESOURCE WALL (check-in point).** Settling faithful-vs-divergent needs the intended semantics of `frame+0x1c` / the on-resume-hook contract, obtainable only by MERT symbolization (re-parse the `17f1_10`/`17f2_10` debug PSP blob; string/assert-xref the context-switch to name the frame fields) or HW register capture of the mgmt Xtensa (likely infeasible). Symbolization is a sizable second-image sub-project -- flagged for Maya rather than launched solo. Everything cheaper has been exhausted this session (iter27-31).

### iter32 ZEPHYR THREAD-MODEL REASONING: the resume hook is a MERT addition that should be null for init; Zephyr's `new==old` guard would CONTINUE init in place, not self-switch (2026-07-10)

Maya's chosen lever (Zephyr thread-model reasoning over cross-silicon Medusa parsing). Validated the framework and derived a concrete, source-grounded divergence.

**17f1_10 validated as a same-FRAMEWORK reference (Maya's caveat confirmed).** It is Xtensa + Zephyr v3.7.1 + MERT (`boards/xtensa/amd_medusa_mpnpu/`, `soc/xtensa/amd_common/mpnpu/`, `*** Booting Zephyr OS build v3.7.1 ***`), but a DIFFERENT board (Medusa/AIE4 vs our Phoenix/AIE2), string-only (no symtab), and the low-level context-switch is assembly with no strings -- so it cannot directly name `frame+0x1c`. Confirmed MERT contexts are Zephyr threads: strings `z_thread_resume`, `switch_handle`, `z_thread_entry`, `z_is_idle_thread_entry`, `arch layer failed to initialize switch_handle`.

**Zephyr `do_swap` (kswap.h) -- the two source-grounded facts:**
1. Stock Zephyr resume = restore regs + **`rfi` to the saved PC**; there is NO per-thread callback in the switch. The fresh-vs-resume distinction is baked into the saved PC (`z_thread_entry` first-dispatch vs `_switch_restore_pc` resume). So MERT's `Callx8 a7`-before-`rfe` in `FUN_00002730` is a MERT ADDITION, not stock Zephyr.
2. `if (new_thread != old_thread) { arch_switch(...) } else { z_requeue_current(old); /* continue, no switch */ }`. When the only runnable thread is the current one, Zephyr does NOT context-switch -- it requeues and RETURNS, so the thread keeps running with no save/restore/hook/rfe.

**Task-struct evidence.** `[0x10f10+0] = 0x12048` (task -> its saved frame); `[0x10f10+12]` (the natural hook/run-fn slot) is NEVER written = 0. So init's MERT on-resume hook is null. The resume trampoline `0xdf8c` even has a `BeqzN a7` null-guard reached via `l32i a7,[a7+12]` -- i.e. a design that loads the hook from a struct+12 and skips it if null. Our path bypasses this: it restores `a7 = frame+0x1c = 0x2450` (stale saved register, faithful windowed-ABI leftover from task_create) and enters at `0xdf98`, PAST the reload+guard.

**Divergence hypothesis (source-grounded):** init cooperatively yields as effectively the sole runnable supervisor thread. Correct MERT/Zephyr behavior = either continue init in place (`new==old` guard) OR resume via a null-guarded hook (`task+12`=0 -> skip -> rfe to saved PC 0x8b043e4 -> RetwN -> 0x3dfc -> keep booting). Our boot instead performs a full self-switch that `Callx8`s the STALE saved a7 (`frame+0x1c=0x2450`) via the UNGUARDED entry. The bug is the hook SOURCE/guard (should be null-sourced `task+12` + guarded `0xdf8c`, or a `new==old` no-switch), not the value 0x2450 itself (that is faithful stale-register).

**Tension to resolve (the one remaining knot):** our traced firmware code deterministically restores `a7=frame+0x1c` and `Call0`s `0xdf98` (verified framing + decode). If that is byte-faithful, real HW would do the same and also crash -- impossible. So EITHER (a) a scheduler decision UPSTREAM (the "is there a different task / should I switch at all" check, MERT's analog of `new!=old`) diverges so we reach this resume when HW would not, OR (b) `frame+0x1c` on HW is not 0x2450 because the SAVE stored a hook there, not the raw a7. NEXT PROBE: trace the scheduler's task-selection/switch decision (the ready-list head==tail `Beq a0,a1` at 0x2a7f and the current-vs-next comparison) to see whether a `new==old`/should-not-switch branch exists and we resolve it wrong -- the direct test of hypothesis (a), and the Zephyr-grounded prime suspect.

### iter33 RUNTIME INSTRUMENTATION -> the switch is REAL (a genuine 2nd task), NOT new==old; iter32's "continue in place" hypothesis is SUPERSEDED (2026-07-10)

Ran the iter32-predicted probe (the `Beq a0,a1 @ 0x2a7f` task-selection watch) plus a full yield call-graph and an at-wall struct dump. New probes: `m2c_probe_yield_callgraph`, `m2c_probe_switch_decision`, `m2c_probe_peek_at_wall` (all self-skip unless `XDNA_FW_PROBE=1`).

**Confirmed the decision structure, exactly at 0x2a7f.** `FUN_00002730+0x34f` is `Beq a0,a1,0x2ae0`. The `a0==a1` branch (0x2ae0) restores PS/EPC1/SAR/loops and `rfe`s **with NO hook**. The `a0!=a1` fall-through advances the list and runs `Call0 0xdf98` = `Callx8 a7` (the 0x2450 wall). So this Beq IS the continue(no-hook) vs switch(hook) fork -- iter32's structural guess was right.

**But the operands demolish the `new==old` reading.** Runtime values at the Beq: `a0 = [0x2b60] = 0x12048` (init's frame, the one being restored, resume-PC `[+0x50]=0x8b043e4`), `a1 = [0x2b64] = 0x15f18`. They are DIFFERENT -> hook path. And `0x15f18` is not spurious:
- `[0x2b64]` (incoming) is set at `+0x301` to `[[0x2278]+0]`. `[0x2278]` (MERT current-task ptr) = **`0x10dfc`**, and `[0x10dfc+0] = 0x15f18`.
- `[0x2278]` was rewritten from init `0x10f10` -> `0x10dfc` at **`FUN_00002730+0x12d` (n=47985)**, mid-yield: the scheduler deliberately makes `0x10dfc` current.
- `0x10dfc` is a **genuine task**, created by `FUN_0000d4a0` at **n=39730** (long before the yield): `[+0x08]=6` (priority, vs init's `0xff`), `[+0x20]=0x08b041bc` (entry), `[+0x00]=0x15f18` (frame). Its entry `0x08b041bc` is **real code** (`AddN`/`Extui`/`Call8`/`J...` at the FULL address; a masked disasm falsely shows zeros -- it lives in the 0x08b04xxx overlay).

So the ready state at the yield is NOT "init alone re-enqueuing itself" (iter26b/28/32). It is **two real tasks**: init (`0x10f10`, prio `0xff`) and a prio-6 task (`0x10dfc`). MERT switches current to the higher-priority `0x10dfc`. `head!=tail` is therefore arguably CORRECT, and the "continue init in place / `new==old`" hypothesis is dead: `new(0x10dfc) != old(init)` is a real switch.

**The wall value is a genuine call-a-data-pointer fault (disprove done).** `0x2450` is zeros in a bss/data gap (surrounding `0x2430..0x2490` all zero); it is init's saved a7 (`[frame 0x12048 +0x1c]=0x2450`, "task_create create-registry ptr"), not code. The prio-6 task's frame `0x15f18` has its a7 slot `[+0x1c]=0` (null) -- so a fresh frame's hook is null, init's is stale-nonzero.

**The new knot (sharper than iter32's).** We RESTORE init's frame `0x12048` (resume-PC `0x8b043e4` = init's syscall+3) while `[0x2278]` current = `0x10dfc`. So we resume init with the prio-6 task queued as "next," and the hook `Callx8(init's a7=0x2450)` walls. Two live root-cause candidates, both empirically testable:
  - **(A) a7-value divergence.** If `head!=tail` is also true on real HW (the prio-6 task really exists there too), then MERT genuinely runs the hook here, so init's saved a7 MUST be a valid function pointer on HW and `0x2450` is our upstream divergence. -> **NEXT: trace init's a7 lineage** back from the frame-save (n=47479) to where init's windowed a7 became `0x2450`, and judge whether that computation is faithful or a leftover we produce wrongly.
  - **(B) wrong-frame-selection.** MERT set current=`0x10dfc` but we restore init's frame (`[0x2b60]` was never advanced to `0x15f18` before the restore -- it flips to `0x15f18` only at `+0x352`, AFTER). If MERT should resume `0x10dfc`'s frame (`0x15f18`, a7=0 -> null hook), we restore the wrong (init) frame. -> test: does the head pointer get advanced too late, or is resuming-init-then-queueing-0x10dfc the intended two-step (init finishes its syscall return before yielding the CPU)?

iter32's predicted probe is DONE and answered its own question: the `new==old` branch exists (0x2a7f) but resolves to a real switch, not a self-continue. The frontier is now the a7 lineage (A) vs the head-advance timing (B) -- both are direct interp traces, no closed-code guesswork.

### iter34 (A) DISPROVEN -- init's a7=0x2450 is FAITHFUL; the wall is a scheduler-STATE divergence (which frame is resumed), not a register-value bug (2026-07-10)

Traced init's `a7` lineage back from the frame-save (`FUN_00002730+0x1f5` = `S32iN a7,[a3+28]`, so the saved value is `FUN_00002730`'s current-window a7 = init's carried a7). Result: **`a7=0x2450` is a faithful scheduler pointer, not garbage.**
- `0x2450 = SCHED+0x200` (`0x2250` is the MERT scheduler base; `a3=0x2250` throughout `FUN_0000d6c0`).
- In `FUN_0000d6c0` (n=47371-47404) `0x2450` is USED as a base pointer -- `L8ui a8,[a7+120]` (ea=`0x24c8`), `S8i [a7+..]` -- i.e. a live, computed pointer into a scheduler sub-block, not an uninitialized-register leftover.
- It is then carried faithfully through the windowed ABI (`FUN_0000d6c0` -> syscall wrapper `0xb041xx/0xb043xx` -> SYSCALL trap -> exception window-spill -> `FUN_00002730` frame-save) into `[frame 0x12048 +0x1c]`.

So init legitimately holds `a7=SCHED+0x200` at the yield; the save/restore/`Callx8` of it is all byte-faithful firmware. **(A) "init's a7 value is our upstream divergence" is DISPROVEN.**

**What that forces.** The firmware faithfully `Callx8`s a real-but-non-code pointer (`0x2450` is a data block; calling it = wall on HW too). HW cannot crash here -> **HW must not reach this `Callx8` for init.** The wall is therefore a SCHEDULER-STATE divergence: we land on the `head!=tail` hook branch (0x2a7f fall-through) where HW takes the no-hook `head==tail` branch (0x2ae0 -> restore SRs -> rfe). The decode is not the issue (Call0 target `0xdf98` is arithmetic-verified; the guarded `0xdf8c` entry that would reload `a7=[a7+12]=[0x245c]=0` and null-skip is NOT what the bytes select).

**The state that must differ (sharpens B).** `tail = [0x2b64] = [[0x2278]+0] = [0x10dfc+0] = 0x15f18`. The resumed frame `head = [0x2b60] = 0x12048` (stale init frame, written once at `+0x260` n=47519, advanced to `0x15f18` only at `+0x352` n=49469 -- AFTER the Beq). If `[0x2b60]` instead held the CURRENT task's frame (`[0x10dfc+0]=0x15f18`), then `head==tail=0x15f18` -> no-hook path -> rfe to `0x10dfc`'s entry (`0x8b041bc`), and boot proceeds. So the prime suspect is **why `[0x2b60]` (the frame the resume restores) is still init's `0x12048` after the scheduler committed current=`0x10dfc`** -- i.e. the pick->commit step (`schedule_next` `FUN_00005958`, which ran n=48014-49430 but never wrote `[0x2b60]`) fails to advance the resume-frame to the picked task. That reconnects to the original iter32 plan (instrument `schedule_next`'s decision + what it commits), now with a specific target: the missing/wrong `[0x2b60]` update. NEXT: trace whether `schedule_next` (or the code between the pick at `+0x12d` n=47985 and the restore at `+0x306` n=49437) is supposed to write `[0x2b60]=picked_frame` and diverges, OR whether resuming-init-then-advancing is the intended design (in which case the hook itself, not the value, is mis-modelled).

### iter35 (B1) EMPIRICALLY CONFIRMED -- the wall is a missing HEAD-COMMIT; poking `[0x2b60]:=picked_frame` DISSOLVES it and boot runs 30k+ new instructions (2026-07-10)

Instrumented the full store-ordering of the three scheduler anchors (`m2c_probe_addr_store_watch` on `[0x2278],[0x2b60],[0x2b64]`) and ran a direct poke experiment (`m2c_probe_head_advance_poke`). Result: the fork converges to **B1**, and B1 is now an OBSERVATION, not an inference.

**Full anchor store-ordering (n, addr, value, site):**
```
n=41463  [0x2278] <- 0x10f10   FUN_00004570+0x13c   current := init
n=47519  [0x2b60] <- 0x12048   FUN_00002730+0x260   HEAD  := init frame
n=47521  [0x2b64] <- 0x12048   FUN_00002730+0x265   TAIL  := init frame   (head==tail at entry)
n=47985  [0x2278] <- 0x10dfc   FUN_00002730+0x12d   current := 0x10dfc    THE SWITCH
n=49435  [0x2b64] <- 0x15f18   FUN_00002730+0x301   TAIL re-derived from current
n=49469  [0x2b60] <- 0x15f18   FUN_00002730+0x352   HEAD advanced -- AFTER the Beq (hook path)
```
Two structural facts fall out. (1) **The switch (`+0x12d`, offset 0x285d) runs at a LATER n (47985) than the head-set (`+0x260`, offset 0x2990, n=47519)** despite the earlier offset -> `FUN_00002730` has a dispatch LOOP, and `+0x260` set `head=tail=init` in a loop pass BEFORE the switch, while current was still init. (2) **`tail` is re-derived from current (`+0x301`: `[0x2b64] := [[0x2278]+0]`), but `head` is NOT** -- it stays frozen at init's frame until `+0x352`, which runs only AFTER the Beq on the hook path. So the compare sees `head=init(stale)` vs `tail=0x10dfc(fresh)` -> differ -> hook.

**The pick is legitimate (B2 argued out).** `+0x12d` is a priority-ordered ready-queue DEQUEUE: `a7=SCHED base 0x2250`, `[a7+40]=0x2278=current`, `[a7+44]=0x227c=prev`. It saves old-current(init) to `[0x227c]` and sets `current := ready_list[prio].head = 0x10dfc` (prio 6, vs init's 0xff). A correct priority dequeue -- 0x10dfc genuinely outranks init, so committing current=0x10dfc is right.

**The hook is unconditionally unguarded, and walls for ANY frame (A fully out).** `+0x356` `Call0 0xdf98` is arithmetic-verified (offset 0x2D44 -> `(0x2a84)+4+0x2D44<<2 = 0xDF98`), landing PAST the `L32iN a7,[a7+12]` reload + `BeqzN` null-guard at `0xdf8c`, straight on `Callx8 a7`. Both `0x0` and `0x2450` are all-zeros in the image -> `Callx8 a7` walls for a7 in {init's 0x2450, a fresh task's 0}. The hook path is a dead end regardless of which frame -> the Beq MUST resolve EQUAL (no-hook 0x2ae0) on HW.

**POKE EXPERIMENT (decisive).** `m2c_probe_head_advance_poke` forces `[0x2b60] := [[0x2278]+0] = 0x15f18` just before the restore reads it (`+0x306`, pc 0x2a36), then lets boot run:
- **Control (no poke):** `Beq a0=0x12048 a1=0x15f18` -> DIFFER -> hook -> **wall at pc=0x2450, n=49473** (the frontier, reproduced).
- **Poked:** `Beq a0=0x15f18 a1=0x15f18` -> EQUAL -> `0x2ae0` (no-hook SR-restore + rfe) -> **boot SAILS PAST the wall**, running 30,527 more instructions (n=49473 -> 80000 budget), final pc=`0x945a`, NO wall / NO spin. Post-Beq PCs confirm the `0x2ae0..0x2aef` no-hook epilogue is taken.

So the 0x2450 wall -- the boot frontier since ~iter18 -- **dissolves with a single state fix: `head` must advance to the picked frame before the restore.** B1 confirmed by observation.

**Root cause, localized (iter36 target).** `head [0x2b60]` is set to init's frame at `+0x260` while current is still init (a dispatch-loop pass BEFORE the `+0x12d` switch), and -- unlike `tail` (`+0x301`) -- is never re-derived from current afterward. The `+0x352` store (`[0x2b60]:=tail`) IS the head-advance, but it sits INSIDE the `head!=tail` branch AFTER the (walling) hook, so it never runs. The fix-POINT is proven (head must == tail before the restore); the remaining question is WHY our run fails to produce it: either (i) an emulator control-flow/loop divergence -- HW's dispatch loop re-runs `+0x260` (or an equivalent head-re-derive) after the switch and ours doesn't, or (ii) an unmodeled head-commit mechanism (a callee / untaken branch between `+0x12d` and `+0x306` that writes the head anchor `*0x2888`). The address-watch proves NO executed store touches `[0x2b60]` in that window, so on HW the commit is on a path we don't take. NEXT: map `FUN_00002730`'s dispatch-loop back-edge (the `+0x265`->`+0x12d` cycle) and find where head should re-derive after the switch.

CAVEAT (honest): the poke is a diagnostic, not a fix; and "runs 30k instructions to budget at pc 0x945a (new territory)" is substantial PROGRESS, not proven boot completion. The natural-boot gate `m2c_bit3_advances_boot_past_natural_wall` still correctly asserts the 0x2450 wall (unchanged boot behavior). New probe: `m2c_probe_head_advance_poke` (gated on `XDNA_FW_PROBE`; `XDNA_FW_POKE=1` enables the force, else control).

### iter36 -- call-tree mapped; head is seeded from the OLD frame BEFORE the pick; B1 CONFIRMED / B2 REFUTED by trajectory contrast (2026-07-10)

Mapped the dynamic call tree of the yield (`m2c_probe_yield_callgraph`, window [47400,49480)) and the entry head/tail-seed (`m2c_probe_disasm_range` on `+0x230..+0x270`), then ran a three-arm poke (`m2c_probe_head_advance_poke`, now modes 0/1/2) to settle B1 vs B2.

**Dynamic call tree (who does what).** The dispatch is `FUN_00002730`; two nested calls do the work:
- `+0x2c5` -> `FUN_0000dab0` subtree -> a nested "pick" helper (the `+0x120..+0x148` block, a standalone windowed fn inside the 0x2730 symbol span) that DEQUEUES the highest-prio ready task and returns **prio 6** (n=47990, a10=6); it commits `current [0x2278] := 0x10dfc` at `+0x12d` (n=47985).
- `+0x2e1` -> `schedule_next` (`0x5974 = FUN_00005958+0x1c`), a per-set-bit loop (`+0x2d0..+0x2e9`, bits of a 4-bit mask) that calls into `FUN_000093f0`/`FUN_00007e4c`/`FUN_0000893c` reading MMIO (`0x25000003`, `0x2505b32c` -- 0x25xxxxxx peripheral space) and returns 0x100. It does NOT write the head anchor.
- Then `+0x301` tail-derive, `+0x34f` Beq, `+0x356` -> `0xdf98` hook -> `Callx8 a7=0x2450` -> wall.

**Head is seeded from the OLD frame, STRUCTURALLY before the pick.** Disasm `+0x256..+0x265`: `a2=[lit 0x2888]=0x2b60`, `[0x2b60]:=a3` (`+0x260`); `a2=[lit 0x288c]=0x2b64`, `[0x2b64]:=a3` (`+0x265`) -- BOTH head and tail seeded from the same `a3` = the old current's frame (init 0x12048). This seed (`+0x260`) is at a LOWER code offset than the pick call (`+0x2c5`) and the tail-rederive (`+0x301`), so in program order **head is always seeded from the outgoing frame before the switch commits.** There is no second head-write on the taken path (only `+0x352`, post-Beq, on the walling hook branch). So the code structure guarantees: **any real switch here makes head!=tail -> the unguarded hook** -- which walls for both candidate a7 values. A switch through this path is therefore only survivable if the OUTGOING frame carries a real switch-hook function in its a7 slot; idle-init (a7=0x2450=SCHED+0x200, a scheduler ptr, not a function) does not.

**B1 vs B2 -- decisive trajectory contrast** (3-arm poke, budget 300k):
| arm | action | Beq | outcome |
|-----|--------|-----|---------|
| control | none | DIFFER | wall 0x2450 @ n=49473 (1 restore pass) |
| **B1** | force `head:=[[0x2278]]=0x15f18` before restore -> resume 0x10dfc | EQUAL | **548 dispatch cycles, ran to 300k budget, no wall**, final pc 0x2afd |
| **B2** | force `current:=init` before tail-derive -> resume init | EQUAL | **wall @ 0xe035 in 38 instructions** (1 restore pass) |

Resuming the picked task (B1) yields sustained RTOS-style operation -- 548 dispatch/switch cycles over ~250k instructions with no wall or spin. Resuming init (B2) dies almost immediately at a fresh wall (0xe035). **B1 confirmed; B2 refuted.** The prio-6 switch to 0x10dfc is REAL and correct; the wall is unambiguously "head fails to advance to the legitimately-picked frame," NOT a spurious pick. (Caveat: 548 cycles-to-budget is healthy PROGRESS, not proven boot completion; B2's quick wall may be partly a crude-poke artifact, but the 548-vs-38 contrast is decisive on direction.)

**iter37 frontier -- the exact head-commit mechanism.** Given the switch is real yet the code seeds head from the outgoing frame with no on-path head-advance, HW must commit head to the picked frame via a path we don't take. Leading hypothesis: the switch-hook (`0xdf98 Callx8 a7`), on a properly-set-up outgoing frame, performs the context-switch bookkeeping (incl. `head:=tail`) and RETURNS; idle-init carries no hook, so either (a) init's frame should hold a real hook function at `[+0x1c]` (upstream: what init loads into a7 before its yield-syscall), or (b) the no-hook idle case should take the GUARDED entry `0xdf8c` (reload `a7=[a7+12]=[0x245c]=0` -> `BeqzN` null-skip -> return) instead of the unguarded `0xdf98`. Both `0x2450` and `0x0` are non-code, so the unguarded call is unsurvivable for idle-init either way -- the frontier is WHY `+0x356` targets the unguarded entry (arith-verified 0xdf98) for a hookless idle task. Probe: `m2c_probe_head_advance_poke` now takes `XDNA_FW_POKE=1` (B1) / `2` (B2).

### iter36b -- steady state proves head==tail is the NORM; NO frame carries a hook; the wall is the UNIQUE idle->first-task current-change (2026-07-10)

Exploited the B1-poked run (548 dispatch cycles after the first, poked switch) to observe how head is committed in NORMAL operation, and dumped the hook slot of every frame. Two clean results.

**Steady state: head==tail for free, because the pick returns the SAME current.** Watching stores to `[0x2b60]`/`[0x2278]` across the poked run: every one of the 548 passes seeds `[0x2b60] := 0x15e78` at `+0x260` and the Beq is EQUAL -- and the value is a CONSTANT `0x15e78` across all passes, while `[0x2278]` (current) is NEVER rewritten after the first switch (n=47985). So after the idle->first-task transition, `current` stays `0x10dfc` and its frame (`[0x10dfc+0]`, now 0x15e78 as the task ran) is re-seeded into head==tail each dispatch. **Normal dispatch never changes current mid-pass, so head==tail is automatic and the hook is never reached.** The wall is the ONE dispatch where the pick CHANGES current (init->0x10dfc, `+0x12d`), and it does so AFTER the `+0x260` head-seed -> head(init)!=tail(0x10dfc).

**NO frame carries a valid hook -- hypothesis (a) REFUTED.** Dumped `[frame+0x1c]` (the saved-a7 / hook slot the `Callx8` targets): init `0x12048`->`0x2450` (scheduler ptr), picked `0x15f18`->`0`, steady `0x15e78`->`0`. None is a function pointer. So the unguarded hook `Callx8 a7` is unsurvivable for EVERY frame, not just init -> the hook is NEVER meant to be reached with these frames -> **head must ALWAYS == tail.** This kills "init should have a real hook" and reframes the whole thing: the divergence is not a7/hook at all, it is that **on the unique idle->first-task transition, `current` must be committed to the target BEFORE the `+0x260` head-seed** (so head seeds to the picked frame and head==tail), and in our run the pick commits it too late (`+0x12d`, inside the `+0x2c5` subtree, after `+0x260`).

**The pick is UNCONDITIONAL, so the "change" is deterministic.** Disasm of the pick helper (`+0x120..+0x148`): `+0x123` `a4=[a7+40]` (current), `+0x125` `BnezN a4` (if current!=0 skip `+0x127`), `+0x129` `[a7+44]:=a4` (save prev), `+0x12b` `a2=[ready_slot+56]`, `+0x12d` `[a7+40]:=a2` (current := ready-queue head). So whenever current!=0 the pick OVERWRITES `current := ready_head`. On the first pass ready_head=0x10dfc != init -> a real change; in steady state ready_head=0x10dfc == current -> no change. The change is not a bug in the pick; it is the correct consequence of init (prio 255) still being `current` while 0x10dfc (prio 6) is already ready-head.

**The decisive logical lever (why this MUST be upstream).** The reconciliation hook (`head!=tail` -> `0xdf98 Callx8 a7`) is unsurvivable for EVERY frame (no frame's `[+0x1c]` is a function). Therefore `head!=tail` is never a state HW reaches here -> a current-CHANGING switch never happens inside this dispatch -> **the init->0x10dfc switch must COMPLETE before this dispatch runs** (this dispatch only ever RESUMES an already-current task, head==tail, like all 548 steady passes). So the divergence is not in the switch epilogue at all; it is that we arrive at this dispatch with `current` still init when it should already be 0x10dfc.

**iter37 frontier (upstream target).** 0x10dfc is CREATED at **n=39730** (`FUN_0000d4a0`, prio 6) while init (prio 255) runs, and init keeps running until its yield at n=47407 -- ~7700 instructions during which a higher-prio task is ready but `current` stays init. The suspect: a **reschedule/preemption that should make 0x10dfc current shortly after creation** (or at least before init's yield reaches this resume-dispatch) is missed in our model, so the switch is DEFERRED into this dispatch (where it walls) instead of completing earlier (where the next dispatch would cleanly resume 0x10dfc). NEXT: trace n=39730 (task-create) forward -- does MERT enqueue 0x10dfc to the ready list, and is there a reschedule point (cooperative yield or preemption) between creation and init's yield where `current` should flip to 0x10dfc? Compare our handling there against what would leave `current=0x10dfc` before the `+0x260` seed. Probe `m2c_probe_head_advance_poke` now also dumps frame hook-slots and watches `[0x2b60]`/`[0x2278]` stores across the run.

### iter37 -- scheduling is COOPERATIVE (preemption hypothesis REFUTED); the divergence is the cooperative context-switch mechanism at init's yield (2026-07-10)

Store-value watch for `0x10dfc` (`m2c_probe_store_value_watch`, budget 48k) finds it written to memory only TWICE before the switch:
```
n=39852  store -> [0x22a0]   FUN_0000d4a0 (creator)   -- enqueue at creation
n=47985  store -> [0x2278]   FUN_00002730+0x12d       -- the pick, at init's yield
```
0x10dfc is placed in a ready slot `[0x22a0]` (SCHED+0x50; the pick reads `current := [ready_slot+56] = [0x22a0]`) at creation and **sits there untouched for ~8000 instructions** until init yields. There is NO intermediate reschedule or preemption. **Scheduling is COOPERATIVE: a higher-prio task created while init runs does NOT preempt init; it waits in the ready slot for init to yield.** This REFUTES the iter36b "missed preemption at task-create" hypothesis -- the switch timing is CORRECT (deferred to init's yield, exactly as cooperative scheduling dictates).

So the divergence is NOT a missed reschedule; it is IN the cooperative context-switch itself. init yields (n=47407) with `current=init` and `ready_head=0x10dfc`; the pick correctly flips `current:=0x10dfc` (`+0x12d`), but this is the ONE current-CHANGING switch, and the switch epilogue seeds head from the OUTGOING (init) frame (`+0x260`, before the pick) with no re-seed after -> head(init)!=tail(0x10dfc) -> unsurvivable hook -> wall. The poke that fixes it (head:=incoming before the restore) is exactly "restore the INCOMING frame," which is what a correct context-switch does.

**The remaining knot (mechanism, no runtime reference exists).** For a current-CHANGING cooperative switch, HW must end with head==tail==incoming (restore the incoming frame). Our code seeds head from the outgoing frame at `+0x260` (structurally before the pick at `+0x2c5`) and never re-seeds. Two framings, both un-refuted:
- **(P) ordering/entry:** on a switching yield HW commits `current` (or seeds head from the incoming task) BEFORE `+0x260` -- e.g. init's yield-syscall picks its successor before entering the resume body, or a switching yield enters `FUN_00002730` at an offset that skips/re-seeds `+0x260`. Our pick-at-`+0x2c5` (after the seed) is then a control-flow/ordering divergence.
- **(Q) bootstrap-abandon:** the init->first-task transition is NOT a normal save-outgoing/load-incoming switch but a "abandon the bootstrap/main thread, hand the CPU to the scheduler" transition where init's frame is discarded and head seeds from the incoming task. init (prio 255) looks like the Zephyr main/idle thread that exits once real threads run.
NO runtime reference for a correct current-changing switch exists in the trace: steady state (548 passes) cycles a SINGLE task (0x10dfc, head==tail always), so the init->0x10dfc switch is the only current-change and it walls. Resolving (P) vs (Q) needs either reverse-engineering `FUN_00002730`'s entry->`+0x260` control flow (does `+0x260` run / seed differently on a switching yield?) or forcing a second current-changing switch to observe the protocol. NEXT: disasm `FUN_00002730` entry (0x2730..0x2830) + `+0x170..+0x230`, and characterize how the handler is entered for a yield vs the branch that reaches the `+0x260` seed.

**iter37b -- the hook target is FAITHFUL (not a guard/overlay bug); the paradox forces "current must be pre-committed BEFORE FUN_00002730 entry".** Ruled out the tempting LOCAL fix (that `+0x356` should call the guarded `0xdf8c` instead of unguarded `0xdf98`): the dispatch epilogue executes at its NATURAL VMA (pc trace 0x2a7f/0x2ae0, not +0x100-shifted) and the static disasm matches runtime exactly, so `0x2a86` is NOT overlaid and `Call0 0xdf98` is faithful (confirmed 4 ways: decode, arithmetic, runtime call tree, no-overlay). So the firmware genuinely calls the unguarded hook, which walls on ANY current-changing switch (every frame's a7 is 0 or 0x2450, never a function). Combined with the code STRUCTURE (head seeded from outgoing at `+0x260`, before the pick at `+0x2c5`), this forces the resolution: **HW must NEVER reach `+0x260` with `current` != the incoming task.** i.e. on a switching yield, `current` is committed to the incoming task BEFORE `FUN_00002730` is entered; the internal pick (`+0x12d`) is then an idempotent no-op (current already == ready_head), and head seeds to the incoming frame -> head==tail -> clean. Our emulator instead enters with `current=init` and flips it at the too-late internal pick -> head(init)!=tail -> wall. So (P)-ordering is the shape, and it is UPSTREAM of `FUN_00002730`: **init's yield/swap path should set `current=0x10dfc` before trapping into the dispatch.** Checked the prime suspect `FUN_0000d6c0` (init's last scheduler call before the yield-syscall, n=47371-47404): it is the priority-indexed READY-QUEUE manager (enqueue/dequeue by prio -- `Addx4` indexing, per-prio head/tail bytes at struct+96/+117, `a3+512` base = SCHED), and it does NOT write `current [0x2278]`. So the current-commit is elsewhere in init's yield path (the `0xb041f0` syscall wrapper -> `FUN_00003df8` -> `0xb043cc` chain, n=47407-47424) OR the swap is structured so the dispatch itself should see current already advanced. NEXT: trace init's yield-syscall path (0xb041f0 / FUN_00003df8) for where `current`/successor is (should be) committed, and/or map `FUN_0000dab0` (the pick caller at `+0x2c5`) to see if the pick is meant to run only when current is already correct.

### iter38 -- iter37b REFUTED by cross-model (Codex) adversarial review; the "pre-commit current upstream" fix is DEAD (2026-07-10)

Codex (different model family, max-effort adversarial pass; brief `CODEX-BRIEF-boot-wall-iter37b.md`, response `CODEX-RESPONSE-boot-wall-iter37b.md`) refuted iter37b with a fact we had in hand but never traced: the THIRD anchor `lit 0x2884 -> 0x2278` (`&current`) and the store at `+0x25b` (pc `0x298b`).

**The decisive store (verified, `m2c_probe_addr_store_watch` on `[0x10f10,0x10dfc,0x2278,0x2b60,0x2b64]`):**
```
n=39746  [0x10dfc] <- 0x15f18   FUN_0000d4a0+0x74   0x10dfc->frame set at creation
n=47517  [0x10f10] <- 0x12048   FUN_00002730+0x25b  current->frame := outgoing (init) frame
n=47519  [0x2b60]  <- 0x12048   +0x260             head := a3 (outgoing)
n=47521  [0x2b64]  <- 0x12048   +0x265             tail := a3 (outgoing)
n=47985  [0x2278]  <- 0x10dfc   +0x12d             pick commits current
```
`+0x25b`/`+0x256..+0x25b` is `a2=[lit 0x2884]=0x2278; a2=[0x2278]=current; [a2]:=a3` = **`current->frame := outgoing_frame`**, executed WHILE `current=init`. iter37b proposed committing `current=0x10dfc` BEFORE this point. But then `+0x25b` would execute `[0x10dfc] := 0x12048`, **destroying 0x10dfc's real frame pointer `0x15f18`** -> both anchors seed to `0x12048` -> the restore selects init, not the incoming task. So pre-committing `current` cannot establish the B1 state; it produces garbage. **iter37b is REFUTED, empirically, not merely unconfirmed.**

**Where iter37b's reasoning went wrong (own epistemic error, per Codex's process critique).** The load-bearing premise "`head!=tail` can never be a HW state" was an over-generalization from a 3-frame sample ("no frame's `[+0x1c]` is a valid hook"). Those three frames are all fresh/zeroed or the bootstrap; the trace contains NO second current-changing switch, so we never sampled a task's frame immediately before a genuine cross-task yield. "This poke runs" was promoted to "HW must create this state by this mechanism." The mechanism then contradicts the `+0x25b` frame-ownership store.

**Codex's better-fitting interpretation (leading hypothesis, NOT yet proven).** The anchors `0x2b60`/`0x2b64` are active/old vs candidate/new context, and `old != new` is an INTENTIONALLY-handled transition, not an impossible state:
1. syscall saves init's outgoing context; `+0x25b` attaches the built frame to `current`(=init).
2. both slots seeded to the outgoing frame.
3. scheduler legitimately picks higher-prio `0x10dfc`; candidate slot refreshed to `0x15f18` (`+0x301`).
4. old frame restored far enough to recover the OLD-context transition callback (`a7`) + args.
5. unequal path commits the new active slot (`+0x352`) and INVOKES the transition callback via the restored old `a7` (the `0xdf98` hook).
6. on callback return, reload the incoming frame (`+0x2a89`) and do a SECOND restore pass (loop `0x2adc -> 0x2a5d`); that pass is equal/no-callback -> `rfe` to `0x08b041bc`.
The wall is at step 5 because init's recovered `a7 = 0x2450` is a DATA pointer, not a valid transition thunk. **Root-cause candidates (Codex, ranked):** (1) bootstrap/adoption callback construction -- init is an already-running boot context adopted by the scheduler and should acquire a valid transition thunk before its first yield but doesn't; (2) a bootstrap special-path that bypasses/guards the callback; (3) register-window/exception-context divergence making the saved `a7` wrong at the save boundary; (4) loader placement -- real `0xdf98` might expose a guarded impl via a still-undiscovered VMA/LMA alias (Codex's residual `+0x50` uncertainty); (5) an earlier scheduler/task-state divergence.

Caveat (our own skepticism applied to Codex): its hypothesis ALSO requires an unobserved thing -- that a correctly-adopted outgoing task carries a valid `a7` callback -- which we've never seen (steady-state task `0x15e78` has `a7=0`). It is the leading un-refuted reading, not a proven one. The whole-trace has ZERO working cross-task switches to learn from.

**Confirmed facts to keep:** `0x2a86` targets `0xdf98` (Codex confirms via independent GNU Xtensa objdump; NOTE this region IS a `+0x100` overlay `CTXSW_CALLEE`, raw file `0x2b86` -- our earlier "not overlaid, natural VMA" was imprecise, though the `0xdf98` target is unchanged); the helper does `Callx8 a7` directly; init supplies `a7=0x2450` with no intervening rewrite; NO hidden direct pre-compare store to `0x2b60` exists (Codex's image-wide literal-xref: 5 refs to the `0x2888->0x2b60` literal, stores only at `0x2990` and `0x2a82`, syscall arm has no CFG edge back to `0x2990`); incoming frame `0x15f18` runs when selected.

**iter39 NEXT -- Codex's highest-value discriminator (single variable, preserves the protocol).** Replace init frame `[0x12048+0x1c]` (its saved `a7`) with the address of a verified ABI-correct no-op windowed thunk (`entry; retw.n`), change NOTHING else (`[0x2278]`, `[0x2b60]`, `[0x2b64]`, the `0x2a7f` branch, the `0x2a86` call all untouched), and observe whether the natural UNEQUAL path: calls+returns through the thunk -> reloads `0x15f18` at `0x2a89` -> reaches the second restore pass at `0x2a5d` -> takes the equal/no-callback path -> `rfe`s to `0x08b041bc`. If yes: the unequal path is a valid switch protocol and the root cause is init's transition-callback/bootstrap CONSTRUCTION (why init lacks a valid `a7` thunk) -- work backward from the required callback contract through task-init/adoption, NOT from the wall forward. This is strictly more discriminating than B1 (which BYPASSES the protocol). Then: cross-image differential (compare the transition tail + initial-frame construction across firmware `1502_00`/`17f0_1x`/`17f1_10`/`17f2_10` by instruction pattern), and pin the exact PSP low-VMA segment map to kill/confirm the `0xdf98+0x50` guarded-byte alias. Do NOT synthesize a scheduler RAM write with no backing instruction/HW mechanism.

### iter39 -- DISCRIMINATOR RAN: Codex's mechanism CONFIRMED empirically; the wall is init's missing a7 transition-callback (2026-07-10)

Probe `m2c_probe_thunk_inject` (gated `XDNA_FW_PROBE=1`; `XDNA_FW_INJECT=1` enables the poke, `XDNA_FW_THUNK=<hex>` overrides the auto-picked thunk). It changes **exactly one word**: init's saved-a7 slot `[0x12064]` (= frame `0x12048 + 0x1c`), rewritten from the garbage `0x2450` to the address of a **verified ABI-correct clean-leaf thunk**. Nothing else is touched -- `[0x2278]` (current), `[0x2b60]`/`[0x2b64]` (head/tail), the `0x2a7f` branch, and the `0x2a86` call all run naturally.

**Finding a real thunk (a rigor detail worth keeping).** First-cut byte-by-byte scan for `entry;retw.n` found `0xc528` = `FUN_0000c500+0x28` -- a MISALIGNED-decode artifact (the real boundaries there are `0xc525 RetwN`, `0xc527 Srli`). Injecting it "worked" (call returned) but then LOOPED in the dispatcher instead of landing in task code, because a mid-instruction `entry` allocates a wrong window. Fix: scan only REAL function boundaries (symbol keys). No pure `entry;retw.n` exists at any boundary, so accept the nearest faithful callback -- a CLEAN LEAF (`entry` -> `retw`/`retw.n` within a few insns, no stores/calls/branches between). Three exist; `FUN_0003b4cc` = `entry a1,32; movi.n a2,0; retw.n` (a side-effect-free return-0 leaf). `FUN_0003b754`, `FUN_0003b808` are equivalent.

**Result (control vs inject, verified thunk):**

| step | control (a7=0x2450) | inject (a7=valid leaf) |
|---|---|---|
| BEQ `0x2a7f` @n=49468 | `head=0x12048 != tail=0x15f18` -> DIFFER -> hook | DIFFER -> hook (**identical -- protocol untouched**) |
| `Callx8 a7` @`0xdf98` | target = `0x2450` (data) | target = thunk (the call fires either way) |
| post-hook `0x2a89` | -- **WALL: Unknown op at 0x2450, n=49473** | **reached** (call returned through thunk) |
| no-hook rfe `0x2ae0` | -- | **reached** |
| rfe lands at | -- | **`0x08b041bc`** -- the incoming task's entry, EXACTLY as predicted |
| after | dead | runs real task code (`0x08b0dxxx`/`0x08b0exxx`) to the 80k budget, no wall |

Thunk-independent (`0x3b4cc` and `0x3b808` give identical control flow, differing only in the trivial +3-instruction count of the leaf body). **So Codex's leading read is CONFIRMED, and it is now EMPIRICAL, not interpretive:** the unequal (`old!=new`) path IS the valid switch protocol -- it intentionally invokes the outgoing task's `a7` transition-callback (hook `0x2a86 -> 0xdf98 Callx8 a7`), returns, reloads the incoming frame, reaches the no-hook rfe, and rfe's into the incoming task. The wall was PURELY init lacking a valid `a7` callback. This is strictly stronger than the old B1 poke (which forged `head` and bypassed the protocol); here the protocol runs untouched and one word dissolves the wall.

**iter37b is doubly dead:** not only was "pre-commit current upstream" refuted by the `+0x25b` frame-ownership store, the actual mechanism (valid `a7` on the OUTGOING frame) is the opposite shape -- the fix is on init's saved register state, not on the scheduler's current/head/tail commit ordering.

**Caveat (do not overclaim):** the return-0 leaf completes the SWITCH but is NOT init's real callback -- so this proves the *mechanism*, not a finished boot. Whatever the real transition-callback was meant to DO is skipped.

**iter40 NEXT -- the sharp fork: is init's `a7 = 0x2450` FAITHFUL to real HW, or an EMULATOR DIVERGENCE?** `0x2450` = SCHED+0x200 (data region), and it is init's *live* `a7` at yield time (init's own boot execution left it in the register file, then the frame-save stored it to `[0x12064]`). Two branches: **(a)** real HW also carries garbage/zero there and the firmware has a guard/path we have not found (then the model is faithful and we mis-read the protocol) -- but iter37b already ruled out a hidden guard on the hook (Codex image-wide xref); OR **(b)** our emulator mis-constructs init's bootstrap/adopted context upstream, and on real HW init's `a7` at this yield is a valid transition-callback (then this is an emulator faithfulness bug in initial-frame/adoption construction). Method: trace BACKWARD from the callback contract -- where does init's `a7` acquire `0x2450`? Watch `a7` writes across init's boot, and find who was *supposed* to install a transition-callback into the outgoing frame's `[+0x1c]` (real tasks presumably carry one; init, the bootstrap-adopted context, does not). Work from the required callback contract through task-init/adoption, NOT from the wall forward. Probe: `m2c_probe_thunk_inject` (`XDNA_FW_PROBE=1 XDNA_FW_INJECT=1`).

### iter40 -- traced a7's origin: it is FAITHFULLY computed; every step checks out yet the wall stands. A structural PREMISE is wrong. (2026-07-10)

Chased the (a)/(b) fork by tracing where init's `a7 = 0x2450` comes from and what the switch contract actually is. Findings, all empirical:

**1. There IS a guard (iter37b's "unguarded hook" was imprecise).** Trampoline `FUN_0000df8c`: `0xdf94 BeqzN a7, 0xdf9f` -- if `a7 == 0`, skip the `Callx8` and return 0. So the contract is explicit: **the outgoing task's saved a7 must be `0` (no switch-out callback -> guard skips) or a valid callable.** Real task frames `0x15f18`/`0x15e78` have `[+0x1c] = 0` -> guard skips cleanly (this is why they don't wall despite the "unguarded" worry). init has `a7 = 0x2450` (nonzero, non-callable) -> guard passes -> `Callx8 0x2450` -> wall. The epilogue's `Call0 0xdf98` enters PAST the guard (at the `Callx8`), but that is moot for init since `0x2450 != 0` would fail the guard anyway. NOTE the guard also means the "guarded 0xdf8c vs unguarded 0xdf98" root candidate is a dead end -- the guard does not save init either way.

**2. Frame layout confirmed.** `[frame+0x00..0x3c]` = spilled a0..a15 (so `[+0x1c]` = a7, offset 7*4=0x1c); `[+0x50]` = saved PC/EPC (init's = `0x08b043e4`, the insn after its yield-syscall; fresh task's = its entry `0x08b041bc`, which is why the post-inject `rfe` landed there). init TCB `0x10f10[+0x20]` = entry = 0 (bootstrap, adopted); task TCB `0x10dfc[+0x20]` = `0x08b041bc`.

**3. a7 = 0x2450 is FAITHFULLY computed, not stale garbage.** Origin trace (`m2c_probe_yield_window`): `FUN_0000d6c0` -- the priority-indexed READY-QUEUE manager (iter37b) -- writes `0x2450` into a7 at `+0xe1` (`0xd7a1`), where `0x2450 = SCHED(0x2250) + 0x200` = the ready-queue array base (`a3+512`). It stays live in a7 (also a2/a5/a8/a10) through init's yield. At the yield-syscall (`0x8b043e1`, WINDOWBASE=5, WINDOWSTART=`0b101011`, PS=`0x60022`): `a7 = 0x2450`. The yield wrapper `FUN_0x8b043cc` (`entry a1,96`) only touches a1/a2/a3, so it does not clear a7. **On real HW, running identical code, a7 would be `0x2450` here too.**

**4. The spill is a straight, faithful register save.** `[0x12064]` (init frame+0x1c) is written EXACTLY ONCE, by the dispatcher `FUN_00002730+0x1f5` (`0x2925`): `S32iN a7, a3, 28` -- part of a plain a2..a12 spill loop (`a3` = frame base). a2/a4/a5 come from EXCSAVE SRs (`Rsr sr213/214/210`, the regs the exception vector clobbered); a6/a7/a8.. are spilled directly (untouched-by-vector = still init's). Confirmed a7=0x2450 in init's window AND in the spill -> faithful.

**5. `0x2450` is data, not code.** Disasm of `0x2440..0x2478`, raw AND with a `+0x100` overlay, is all zeros/`Unknown`. `0x2450` = ready-queue array base (RAM data), never code. So `Callx8 0x2450` genuinely walls -- on HW too, if reached.

**6. The syscall vectors to REAL firmware handler code, not an emulator shim.** `Op::Syscall -> raise_general_exception(EXCCAUSE_SYSCALL) -> VECBASE + general-exception offset`. The entire save/restore path is firmware code our interp executes.

**THE PARADOX.** Every step is faithful to the firmware: init computes `0x2450` into a7 (ready-queue base), yields, the dispatcher spills a7 -> frame+0x1c, the epilogue loads it, the guard (`a7 != 0`) passes, `Callx8 0x2450` -> wall. Yet the SAME firmware boots on real silicon. So one of my structural PREMISES is wrong, and it can no longer be settled by more emu-probing (I've hit this from four angles) or by HW (Xtensa mgmt-core register state mid-boot is not observable). Leading premise-suspects: **(i)** a subtle window/SR emulation divergence in the exception save/restore (the asymmetric `Rsr`-vs-direct spill, or a windowbase/windowstart handling difference) leaves a7=`0x2450` where HW's handler would present `0`; **(ii)** head/tail semantics -- the restored/called frame is the OUTGOING (init) frame, which is structurally odd for a "switch-out callback"; **(iii)** init is special (scheduler/main context, not a task) and its a7 at yield should be a continuation the code we run never installs. **iter41 NEXT = cross-model (Codex) adversarial review of the paradox** -- this is the "interpretive claim as fact with no human/HW oracle" trigger. Probes: `m2c_probe_yield_window`, `m2c_probe_thunk_inject`, `m2c_probe_addr_store_watch` (all `XDNA_FW_PROBE=1`).

### iter41 -- Codex round-1 verdict (premise #5: I-side code at 0x2450) REFUTED by byte-recovery; paradox reopened, round-2 dispatched (2026-07-10)

Ran the Codex adversarial review (brief `CODEX-BRIEF-boot-wall-iter40.md`; new plugin subagent `codex:codex-rescue`; max-effort GPT-5.6). **Codex's verdict:** the false premise is my #5 -- "`0x2450` is a scheduler DATA address, therefore instruction fetch at `0x2450` cannot be code." On this Harvard Xtensa, D-side `0x2450` is scheduler RAM but I-side `0x2450` is a separate object; it argued real PSP loading places an executable switch-thunk at I-VMA `0x2450` that our incomplete/hand-bounded I-side map misses -- the SAME class as the `+0x100` Harvard sections (`0x880`, `0xdac4`, `0xe1fc`) we mapped by walk-and-stub. It ruled (i) window-divergence OUT (concrete trace: WB stays 5, WS stays `0x2b`; general-exception entry touches EPC1/EXCCAUSE/PS.EXCM/PC but never WINDOWBASE/WINDOWSTART/ARs; matches Zephyr's Xtensa vectors), (ii) outgoing-frame reading TRUE, (iii) bootstrap-callback OUT. It also correctly noted the `BeqzN a7` guard is NOT executed (`Call0 0xdf98` enters past it). It was honest that the actual I-side backing bytes were "not yet recovered."

**I verified the precedent (real -- the `+0x100`/Harvard section bug is our most-hit boot class) and then went to recover the bytes. The recovery REFUTES Codex's mechanism:**
- **No code backs VMA `0x2450` under either relocation delta.** Image `1502_00/npu.dev.sbin` (size `0x3cb10`) uses only base `+0x5c` and `+0x100` overlays. Raw bytes at file `0x24ac` (=`0x2450+0x5c`) and file `0x2550` (=`0x2450+0x100`) are both zero-deserts (2 nonzero bytes in 32, sparse noise -- not a function).
- **`0x00002450` occurs ZERO times as a 32-bit literal in the whole image.** Every real `+0x100` code section was found because a compiled-in pool pointer referenced its entry; `0x2450` is referenced by nothing -- it is never a designed code entry.
- **`0x2450` is provably DATA.** `FUN_0000d6c0+0xde` (`0xd79e Addmi a7,a3,512` = SCHED+0x200) then `0xd7a1 L8ui a8,a7,120` / `0xd7a7 S8i a4,a7,120` -- a BYTE load/store at `[0x24c8]`. The per-priority ready-queue array base. `a7=0x2450` is data-pointer residue.

Also nailed the switch SEMANTICS: `0x2a5d..0x2a73` is a bulk register RESTORE (`a4..a15 := [a3+16..60]`), so `a7 := [a3+0x1c]` is just register 7 of the restored OUTGOING (init) frame, NOT a dedicated callback slot. So the design restores the outgoing frame and calls its residual a7. (Caveat held: the refutation assumes no THIRD, unprecedented relocation delta for this VMA.)

**Reframed paradox (premise #5 dead):** `a7=0x2450` is faithful data residue, no code backs it, the `Callx8 a7` genuinely targets a non-code address, yet the identical firmware boots on silicon -> on HW `Callx8 a7` must NOT run with `a7=0x2450` here. By elimination: **(A)** a7 should be `0`/callable -- a window/register-RESIDUE divergence (boot takes ~114 `0x880` overflows; if a spill/fill would overwrite physical AR27 between n~47394 and the hook n~49472 but our model does not, HW's a7 differs -- Codex's suspect (i), to be re-examined WITH the overflows, not assuming none); **(B)** the hook should not fire (head/tail or frame selection); **(C)** the `Call0` target/trampoline entry is wrong; **(D)** an unenumerated premise. **iter41b = Codex round-2** (brief `CODEX-BRIEF-boot-wall-iter41.md`, dispatched): armed with the #5 refutation, sic'd open-ended on the reframed paradox, led toward the AR27/window-residue thread. Rationale (Maya): Codex is cheap + competent at RE grind; reserve expensive Opus for framing + skeptical judgment of what returns. Verify its next killer fact before accepting (round-1's did not survive).

### iter42 -- RESOLVED. The `0x2450` wall was a MISFRAMED TRAMPOLINE: `0xdf98` is a `+0x100` window-rotation helper we fetched at `+0x5c` as a bogus `Callx8 a7`. There was never a "hook." (2026-07-10, Codex round-2 + independent verification)

**The whole "switch-out a7 callback" mechanism (iters 35-40) was a DISASSEMBLY ARTIFACT.** The dispatcher epilogue's `0x2a86 Call0 0xdf98` is correct, and `0x2a86` executes in the `+0x100` CTXSW section. But VMA `0xdf98` sits in the GAP above the `SYSCALL_BLOCK` overlay (`0xd8a7..0xde04`), so we fetched it at base `+0x5c` (file `0xdff4` = bytes `e0 07 00` = `Callx8 a7`). The REAL linked target is file `0xe098` (the same `+0x100` framing as its caller), which is a **non-windowed WINDOWBASE/WINDOWSTART rotation helper** -- independently disassembled (overlay applied):
```
0xdf98 Rsr a2, WINDOWBASE(72) ; AddiN a2,a2,1 ; Ssr a2 ; Rsr a3, WINDOWSTART(73)
       Srl a2,a3 ; Sll a3,a3 ; Bgez ... ; Extui ; Or a2,a2,a3 ; Wsr a2, WINDOWSTART
       Neg ; And ; Nsau a2,a2 ; ... ; Rsr/Srl/Wsr WINDOWSTART ; ... ; ret.n
```
This is textbook Xtensa software window-spill/rotation, ending at the `ret.n` near `0xe0af`; file `0xe1b1` (VMA `0xe0b1`) turns to zero padding, bounding the section. It returns cleanly to `0x2a89` -- it NEVER calls `a7`. `a7=0x2450` (faithful ready-queue-pointer residue) was never supposed to be executed; the misframe manufactured a `Callx8 a7` that walled on it.

**Fix (Codex, `src/firmware/mod.rs`, verified by me):** one `+0x100` overlay `CTXSW_WINDOW_ROTATE = [0xdf98, 0xe0b1)` -- the same walk-and-stub `+0x100` section mechanism as `CTXSW_CALLEE`/`SYSCALL_BLOCK`. No Xtensa instruction semantics changed (`Call0`/`Rsr`/`Wsr`/`Rotw`/`RetN` were already correct). Plus a regression test tying `Call0 @0x2a86 -> 0xdf98 = Rsr WINDOWBASE`, and a genuine bug-fix to `m2c_probe_stack_leak`'s window-vector lists (they were mislabeled -- overflow was `[0x800,0x840,0x880]`, correct is `O={0x800,0x880,0x900}, U={0x840,0x8c0,0x940}`; **this is why iter41's "114 overflows" was bogus -- the probe counted the U4 vector `0x840` as an overflow**; corrected, the whole boot has 0 overflow / 0 underflow, which also independently kills suspect (A)).

**Independent verification (mine, not Codex's):** (1) `0xdf98`@`+0x100` disassembles as the window helper above, not `Callx8 a7`. (2) Natural boot -- `m2c_probe_thunk_inject` with NO injection -- runs `Call0 0xdf98` (n=49471) -> helper returns to `0x2a89` (n=49553) -> no-hook rfe `0x2ae0` (n=49586) -> incoming task entry `0x08b041bc` (n=49624) -> executes Seg-B task code to the 80k budget (final pc `0xb0e794`). The `0x2450` wall is gone by the overlay ALONE; iter39's a7-inject was patching the symptom. (3) `cargo test --lib`: 4100 passed / 0 failed; new regression test green; fmt clean.

**Suspect scorecard (final):** the true premise was **(C), refined -- correct `Call0` arithmetic, wrong relocation FRAMING of the target** (not "bad decoder math"). (A) window-residue: ruled out (0 overflows once the probe vectors were corrected; the `0x880` handler's `S32e` only reads AR27, never writes it). (B) head/tail: ruled out (`0x2b60/0x2b64` are old/new frame anchors; the protocol is intentional and, with the helper mapped, reaches `0x08b041bc` untouched). Premise-#5 (I-side code at `0x2450`): stays refuted -- `0x2450` is data; the fix is at the trampoline `0xdf98`, not `0x2450`.
...
### iter44 -- GATE PINNED: the firmware has BOOTED and is idling on an EMPTY external completion ring (2026-07-10)

Drilled the iter43 steady-loop to its root, layer by layer (all via `m2c_probe_tail_poll` + `m2c_probe_disasm_range` + `m2c_probe_addr_store_watch`):

1. **The "state 0x6c" is a SYSCALL SELECTOR, not a state-machine field.** The value `dab0` reads (`[a2]=0x6c` @`0x15f60`) is written each period by a **syscall wrapper** in Seg-B at `0xb0424c`: `Entry a1,96; Movi a3,0x6c; S32iN a3,[a1+16]; Wur UR231,&argblock; Syscall; RetwN`. No return value is loaded after `Syscall` -- **syscall 0x6c is VOID**. `dab0`'s `k`/`m`/`l`/`p` (`0x6b`/`0x6d`/`0x6c`/`0x70`) comparison tree is the **kernel syscall dispatcher**, and `0x6c` is the call the first task re-issues every period (arg = fixed buffer `0x15fc0`, `caller_a0=0`).

2. **The kernel dispatch for 0x6c is an event-service scan driven by a CONFIGURED active-set.** The dispatcher `FUN_00002730` reads pending-mask `[0x272003b8]=0x8000`, extracts `(0x8000>>12)&0xf = 8 -> bit 3 -> service index 3`, and for each set bit runs the service chain `FUN_00005958 -> 93f0 -> 9448 -> 7e4c -> 893c`. A store-watch proves `[0x272003b8]` is **written only 4x during init** (`0x1000/0x2000/0x4000/0x8000`, progressive, at the page-map helper `0x89ad` D-side) and NEVER after -- so `0x8000` is a stable CONFIG value (which columns/channels are active), not a self-clearing HW event flag. It does not "fail to clear"; it is correctly persistent.

3. **The service handler scans an EMPTY completion ring.** `FUN_000093f0` (the ring-scan) loads a ring's head/tail (`[a5+0x200]`/`[a5+0x204]`, backed by the aperture reads `0x27200330`/`0x2720032c` = **0/0**), finds `head==tail` (empty), and returns with nothing to process. The `0x27220000` reads (786x) are a separate **page-mapping** read-modify-write helper (`Beq loaded,expected ? skip : S32iN+Memw`), not a poll -- confirmed by the `Slli 12/20`/`&0xfff` page arithmetic.

**Conclusion: this is not a wall or a bug -- the firmware has BOOTED and is idling.** The first task's steady loop issues a void "process events" syscall; the kernel scans the configured active columns' completion rings; the rings are empty because we (the host/array) supply no events; so it finds no work and loops. Progress to go-alive (column power-up / `publish_chann_info`) is **host-triggered**, not autonomous -- the firmware is correctly waiting for us to be the host. The boot-to-idle mission's goal (run real mgmt firmware to a coherent idle without hardcoding magic timings) is **reached**: the boot ran entirely on the firmware's own code, and the "idle" it reaches is this poll-loop (this firmware idles by polling empty rings, not by `waiti`).

**NEXT ARC (separate phase, not part of boot-to-idle):** drive go-alive by injecting a host command / completion into the ring (`0x27200330`/`0x2720032c` head/tail + a ring entry) and watch the firmware pick it up, power up columns, and `publish_chann_info`. That is the start of building out the host<->firmware ring/doorbell contract -- a fresh arc, opened deliberately, not a continuation of this wall-breaking one.
