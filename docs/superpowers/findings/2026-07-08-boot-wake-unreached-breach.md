...
**iter37 frontier (sharpened, upstream).** WHY is `current` still init at `+0x260` on the first transition? Either HW pre-commits `current=0x10dfc` in init's yield-syscall path (before entering this dispatch body), or the idle->first-task handoff uses a different entry that seeds head from the picked task. NEXT: trace init's yield-syscall -> dispatch entry (the `FUN_00003df8`/`0xb043cc` path in the call tree, n=47407-47548) to find where `current` is/should-be set relative to the `+0x260` seed -- the pick-vs-seed ordering is the root. Probe `m2c_probe_head_advance_poke` now also dumps frame hook-slots and watches `[0x2b60]`/`[0x2278]` stores across the run.
