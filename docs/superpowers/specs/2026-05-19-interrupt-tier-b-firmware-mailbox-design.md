...
| **`err_code` bit layout** | 5 fields, 4-bit masks (except `num` at 16): `num` [15:0], `driver` [19:16], `severity` [27:24], `module` [35:32], `class` [43:40]. Tier B uses the convenience `build_critical_aie_error_code(num, module)` which hardcodes `driver=Aie`, `severity=Critical`, `class=Aie` — matching the driver's `AMDXDNA_CRITICAL_ERROR_CODE_BUILD` macro path (`amdxdna_error.h:113-115`, used at `aie2_error.c:239`). | Faithful to the driver wire format — encoded `err_code` bytes match exactly what a real driver writes for AIE async errors. Without this, any consumer that decodes via the driver macros (XRT host library or a future kernel-driver shim attaching to our plugin) would mis-parse the cache. Enum discriminants for `AmdxdnaErrorNum` / `AmdxdnaErrorModule` / `Severity` / `Class` / new `AmdxdnaErrorDriver` mirror the driver enum values verbatim. |
