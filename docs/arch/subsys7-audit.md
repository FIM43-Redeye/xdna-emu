...
- **Prescribed migration verb.** `move-to-archspec` (for the constants); algorithms stay `leave-alone`. The cascade width (384 bits), FIFO depth, and register address are archspec data. The existence vs. non-existence of the cascade link is an AIE1/AIE2 divergence, but a feature flag (`has_cascade_link: bool`) readable via `arch_handle::processor_model()` captures it without a trait method -- the execute path gates on the flag. AIE1 simply has no cascade (not a different cascade), so the shape-vs-values test lands on values + feature flag, not on trait method.
...
- **Prescribed migration verb.** `leave-alone` for the arithmetic algorithms; the `aie2_acc_fp32_add` function in `vector_float.rs` encapsulates AIE2-specific float behavior and is already factored out. The NaN behavior is AIE2 hardware ground truth; no constant to migrate. The 14 AIE2 hardcodes here are behavioral facts (NaN canonicalization, fp32 accumulator paths), not parametric values -- AIE1's equivalent behavior would differ in kind, not in a parameter, so a future AIE1 port handles these via a feature flag or `#[cfg]` guard rather than a shared parameterizable algorithm.
- **Estimated LOC impact.** 0.
...
- **Prescribed migration verb.** `read-archspec-via-accessor` (after `vmac_routing.rs` moves to archspec, the call sites in `vmac_hw.rs` read via the archspec module path); `move-to-archspec` for the mmode/pmode encoding table in `vec_control()` (data constants move; the `vec_control()` algorithm itself stays in xdna-emu).
...
| Cascade data (`CASCADE_WORDS = 6` + new `has_cascade_link: bool` flag) | `cascade.rs` (currently hardcodes width + presence implicitly) | move-to-archspec | `aie2::processor` | −1 in execute, +2 in archspec |
