...
| A DMA channel's retry is whole-mask, so a multi-bank DMA demand has no starvation bound | The AM020 ch.2:166 retry contract is honoured per BANK for the core (a port that wins bank 0 and loses bank 1 keeps bank 0 and re-requests only bank 1 -- `CoreInterpreter::bank_served_banks`). A DENIED DMA CHANNEL instead re-presents its whole mask, because `step_with_denied` skips its FSM step entirely. For a single-bank demand -- which is every MM2S granule fetch by construction, and every ordinary 1-word S2MM cycle -- the two are identical. An S2MM cycle whose words straddle a 16-byte granule boundary (unaligned BD base, or a stride that lands words in separate granules) can declare TWO banks, and a whole-mask retry of a multi-bank demand has NO round-robin starvation bound: two rotors can hold a relative phase in which the requester never wins both banks in one cycle. | `src/device/dma/engine/stepping.rs` (`channel_bank_mask`, `is_held_inert`), `src/device/bank_arbiter.rs` (module doc) | **OPEN, never observed, pinned by test.** `bank_arbiter::multi_bank_requester_starves_without_per_bank_stickiness` proves the whole-mask retry starves at rotor skews 2 and 3, and the module doc states the bound holds only for per-bank retriers. Not triggered by any current kernel (no multi-bank DMA demand has been observed), and the fix is structurally larger than the core's: a partially-served DMA cycle means committing SOME of the cycle's words and holding the rest, where today a denied channel is held wholly inert. Found in the final review of the memory-stall arc (2026-07-14). |
