...
| Gap | Model vs hardware | Where | Status / rationale |
|-----|-------------------|-------|--------------------|
| `DMA_S2MM_n_MEMORY_BACKPRESSURE` never fires | EMU 0 cycles; HW 756 (ConsA) / 853 (ConsB) on the `of_q0_rich` capture | `src/interpreter/engine/coordinator.rs` (Phase E, where the wiring used to be) | **OPEN, deliberate, HW-gated.** It was previously wired to bank-arbitration denial. Hardware disproved that mechanism: an MM2S that lost 112 bank arbitrations reported `MEMORY_STARVATION = 0` and paid 5 cycles total for the losses, because its 128-bit staging buffer has three spare cycles in four ([`2026-07-14-dma-bank-access-width.md`](../superpowers/findings/2026-07-14-dma-bank-access-width.md)). The event tracks the channel's *data-path* inability to move data (input FIFO full), not arbitration. Emitting 0 is more honest than a wrong mechanism producing a coincidentally non-zero number. The staging-FIFO model that does drive it is gated on a hardware capture. |
| `DMA_MM2S_n_MEMORY_STARVATION` never fires | EMU 0 cycles; **HW 0 cycles** in every variant of the bank-width capture -- this one now MATCHES | same | **CLOSED for the arbitration case.** The emulator previously emitted 40/53/128 cycles here purely on arbitration loss, against HW's 0. Removing that wiring makes the model agree with silicon. Whether the event fires at all under a genuinely starved output FIFO is untested (no capture yet drives one). |
