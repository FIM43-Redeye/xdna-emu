...
| Gap | Model vs hardware | Where | Status / rationale |
|-----|-------------------|-------|--------------------|
| `DMA_S2MM_n_MEMORY_BACKPRESSURE` total inflated by a warmup lock stall | Steady-state windows match silicon; the RUN TOTAL does not: EMU 8544 (ConsA) / 8390 (ConsB) vs HW 756 / 853. Excluding one anomalous window: EMU **840 / 782** vs HW **756 / 853**. | `src/device/dma/engine/stream_io.rs` (the ingress FIFO), `src/interpreter/engine/coordinator.rs` (Phase E) | **The EVENT is CLOSED; the residual is NOT a FIFO gap.** The event is now driven by the S2MM ingress FIFO (`note_ingress_offer_refused`: a beat offered, the FIFO full), with nothing consulting the lock or the arbiter. HW's shape reproduces emergently: steady-state windows open **+16** cycles into a lock stall (HW +15, zero variance) and hold **101** cycles of a **117**-cycle window (HW 104 of 119); cold-start and stream-drained windows correctly assert **nothing** (HW: same); every window is sustained, zero single-cycle pulses. The +16-vs-+15 is exactly the depth-16 FIFO taking 16 beats where silicon takes 15 -- NOT fitted away. The 10x total is one pre-existing pathological lock stall (7784 / 7660 cycles, vs silicon's 1174-cycle cold-start window) during which the FIFO faithfully backpressures; it predates this model (confirmed: same 7754-cycle stall gap on the parent commit) and belongs to the emulator's first-output warmup, not to the DMA. |
