...
status: encoding layer CLOSED (2026-06-27); overrun encoding fixed, trigger open
---
...
| Trace overrun status and transition | Phoenix physically reported core `Trace_Status=0x300`; AM025 defines state `11` as overrun. The emulator had no overrun state and its queues do not yet produce one. | `src/device/trace_unit/mod.rs`; [finding: 2026-08-07 Phoenix trace overrun status](../superpowers/findings/2026-08-07-phoenix-trace-overrun-status.md) | **ENCODING FIXED / TRANSITION OPEN.** Architectural status now encodes Overrun as 3 and drains it as terminal. The qualified treatment snapshots `0x300` before the gate, after restore, and after stop, localizing this observed overrun before the gate and excluding the transition/restore/stop wave as its cause. Exact capacity/backpressure remains unproved, so no trigger was invented. |
