...
| **Core compute timing** | [`fidelity-gaps/core-compute-timing.md`](fidelity-gaps/core-compute-timing.md) | needs-HW-empirical | MEMORY_STALL per-access stall (open, own campaign); lock-arb cost modeled, pulse emission pending; bank-arbiter resume-cycle hole (open, dormant/unreachable on corpus) |
