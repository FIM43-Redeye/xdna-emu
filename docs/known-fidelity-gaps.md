...
| **Host / firmware dispatch** | [`fidelity-gaps/host-firmware-dispatch.md`](fidelity-gaps/host-firmware-dispatch.md) | off-array / firmware seam | dispatch latency (deferred), core reset-deassert (**root-caused**, Part 1 landed `1e9e6700`), functional partition-memory zeroization (**fixed**, latency open), firmware-selected device-heap translation, `CONFIG_CU` / PDI handoff, and dual-compiler unmodified-driver round trips through chained, direct-CU, and observable transaction-ELF direct-DPU execution (**fixed**); signed-firmware CDO timing disproves scalar interpreter timing, and `BlockWrite(1)` now has a confirmed transaction-phase law absent from the functional scheduler, leaving an **open causal timing gap** |
