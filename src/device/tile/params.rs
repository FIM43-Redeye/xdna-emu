...
/// `bank_arbiter::NUM_DMA_CHANNELS` the arbiter itself uses, so this can
/// never silently drift from the real per-direction channel count.
pub(crate) const DMA_BANK_REQUESTER_COUNT: usize = 2 * crate::device::bank_arbiter::NUM_DMA_CHANNELS;
