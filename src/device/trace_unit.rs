    ...
    pub fn write_register(&mut self, offset: u32, value: u32) {
        match offset {
            0x00 => {
                ...
                // On real hardware, tracing starts when the start event
                // (typically BROADCAST_15) is generated by the CDO via
                // Event_Generate registers. Since we don't model the broadcast
                // event system, auto-start the trace unit on configuration.
                // The CDO always triggers the start event before cores begin.
