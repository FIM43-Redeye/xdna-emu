...
impl EventModule {
    ...
    /// broadcasts `GROUP_ERRORS_0`. Compute-memory, shim-PL, and mem-tile
    /// members follow their respective single `GROUP_ERRORS` event. All IDs
    /// come from the generated event table.
    pub(crate) fn triggered_error_group(&self, event_id: EventId) -> Option<EventId> {
        use xdna_archspec::aie2::trace_events::{core_events, mem_events, memtile_events, shim_events};
        ...
        let (member_base, group_id) = match self.module_type {
            ...
            EventModuleType::Pl => (shim_events::GROUP_ERRORS, shim_events::GROUP_ERRORS),
