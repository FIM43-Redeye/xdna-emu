...
pub fn run_on_hw_and_validate(
    ...
    let result = npu_runner::run_on_npu(spec, test_name, xclbin_path, insts_path, DEFAULT_HW_TIMEOUT_SECS)
        .map_err(|e| {
...
pub fn run_hw_and_print(
    ...
    let result = match run_on_hw_and_validate(spec, test_name, xclbin, insts, reference_dir) {
        Ok((label, output)) => {
            ...
            let outcome = if label.starts_with("PASS") { HwOutcome::Pass } else { HwOutcome::Fail };
            if compact {
                print!("{}: {:18} ({:.1}s)", prefix, label, elapsed);
            } else {
                println!("      {}: {} ({:.1}s)", prefix, label, elapsed);
            }
            HwRunResult { outcome, label, output, elapsed_secs: elapsed }
        }
        Err(e) => {
            let elapsed = hw_start.elapsed().as_secs_f64();
            let (outcome, label) = if e.wedged {
                ...
                (HwOutcome::Error, format!("ERROR ({})", &meaningful[..meaningful.len().min(60)]))
            };
            if compact {
                print!("{}: {}", prefix, label);
            } else {
    ...
    if result.outcome != HwOutcome::Wedged {
        let is_error = result.outcome == HwOutcome::Error;
