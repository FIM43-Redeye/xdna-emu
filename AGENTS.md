...
Machine-specific environment state and recovery procedures live in
[`docs/operations.md`](docs/operations.md).

### Toolchain discovery

Activation is a convenience, not a correctness requirement. Builds and required
runtime queries resolve toolchains identically from the main checkout and linked
worktrees:
...
Explicit configuration is fail-closed: blank, missing, or incomplete paths are
errors and never fall through. Automatic discovery skips incomplete shadow
directories and continues upward. `AIE_RT_PATH` names `driver/src`, not the
repository root.

The host LLVM used by `tblgen-rs` is separate from Peano. By default it finds
system LLVM 21 on `PATH`; `TABLEGEN_210_PREFIX` remains an explicit host-LLVM
override. The resolved llvm-aie tree supplies AIE TableGen sources and decoder
libraries. The workspace `.cargo/config.toml` deliberately sets neither path.
...
1. **aie-rt** (`driver/src/` under the resolved aie-rt tree, branch
   `xlnx_rel_v2025.2`) -- the same HAL that programs real silicon. Reference
   implementation for DMA, lock, and stream-switch behavior. Use the official
   Xilinx clone, NOT mlir-aie's vendored fork.
2. **AM025 register database**
   (`lib/Dialect/AIE/Util/aie_registers_aie2.json` under the resolved mlir-aie
   tree) -- 1,806 registers / 6,412 bit fields, parsed by `regdb.rs`. BD field
   parsing is already fully data-driven from this (zero hardcoded bit positions
   in `bd.rs`); extend the pattern.
3. **llvm-aie TableGen** (`llvm/lib/Target/AIE/` under the resolved llvm-aie
   tree) -- complete ISA. Decoding is fully TableGen-driven; semantics ~33%
   data-driven via SemanticOp, closing the gap is the goal.
...
**Research guidance**: these sources are extensive. Navigate them with targeted
searches and narrow reads. In Codex, keep the work on the primary agent unless
Maya explicitly requests subagents.
...
- **aie-rt**: resolved `driver/src/` - Official Xilinx HAL (branch
...
- **mlir-aie**: resolved repository root - AIE compiler, test binaries, device
  models, AM025 register database JSON
- **llvm-aie**: resolved repository root - Peano compiler, ISA definitions via
  TableGen
