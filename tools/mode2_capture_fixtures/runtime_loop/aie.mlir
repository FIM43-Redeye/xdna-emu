// Mode-2 fixture aie.mlir: runtime_loop.
//
// Single-tile kernel that calls k_runtime_loop once per host invocation.
// 64 i32 are streamed from DDR -> shim -> compute via of_in; one i32 result
// is streamed back via of_out (we ship the full 64xi32 buffer but only
// out[0] is meaningful).
//
// The placeholder NPUDEVICE token is replaced by a per-arch sed line in
// build_fixture.sh (npu1_1col for AIE2, npu2_1col for AIE2P).
...
module {
  aie.device(NPUDEVICE) {
    ...
    func.func private @k_runtime_loop(memref<64xi32>, memref<64xi32>) attributes {link_with = "kernel.o"}
    ...
    aie.core(%tile_0_2) {
      ...
      func.call @k_runtime_loop(%elem_in, %elem_out) : (memref<64xi32>, memref<64xi32>) -> ()
