...
#     Hardware-fixed to r16-r19 (callee-saved, handled by prologue/epilogue).
#   MvBMXDst (bw=6)
    ...
    if kind == "scalar":
        if operand_type == "register+16":
            if _needs_register_pair(instr_name):
                # eL register pair class.  Use only l4-l7 (r25:r24 through
                # r31:r30) which are caller-saved.  l0-l3 (r17:r16 through
                # r23:r22) are callee-saved per AIE2 ABI.
                return ["r25:r24", "r27:r26", "r29:r28", "r31:r30"]
            # eRS8 high scalar class (r16-r23).  ALL are callee-saved per
            # AIE2 ABI, but the hardware encoding forces this range (3-bit
            # field + offset 16).  The kernel prologue/epilogue saves and
            # restores these registers to maintain ABI compliance.
...
def build_mega_program(test_points: list[str], out_size: int = 0) -> str:
    ...
    """
    # Callee-saved registers per AIE2 ABI (AIE2CallingConv.td):
    #   r16-r23, p6, p7, lr
    # Some test instructions are hardware-constrained to write these
    # (eRS8/eL/ERS4 register classes).  Save on entry, restore on exit.
    # lr is preserved by main()'s stack save/restore; p6/p7 are excluded
    # from test register assignment.  We save r16-r23 here.
    CALLEE_SAVED = [f"r{i}" for i in range(16, 24)]
    ...
    lines.append(f"  paddb [sp], #{frame_size}")
    for i, reg in enumerate(CALLEE_SAVED):
        lines.append(f"  st {reg}, [sp, #{-(i + 1) * 4}]")
