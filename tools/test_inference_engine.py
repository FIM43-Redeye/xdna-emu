...
def test_engine_backward_compat_no_dump(tmp_path):
    # Existing callers that don't pass a dump still work; connectivity now runs
    # without a dump (uses candidate pairs). dump=None means count-truncation
    # ceiling isn't derivable -> count_ceiling_unknown remains.
    dirs = _cross_domain_dirs(tmp_path)
    led = _ledger(tmp_path, [
        {"cite": "program:x", "a": "1|0|2|MM2S", "b": "1|2|0|CORE", "kind": "program"}])
    rep = run_engine(dirs, led, [("1|2|0|CORE", "1|0|2|MM2S")])
