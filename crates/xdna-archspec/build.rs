...
//! - `gen_arch.rs`            -- top-level AIE2 constants (COLUMNS, ROWS, etc.)
//! - `gen_stream_ports.rs`    -- stream switch port type arrays
//! - `gen_subsystems.rs`      -- per-tile-type subsystem address ranges
//! - `gen_core_module.rs`     -- core module register offsets
...
fn main() {
    ...
    // mlir-aie bridge path (for trace events generation).
...
///
/// The source file lives at `crates/xdna-archspec/decoder_ffi/aie2_decoder.cpp`
/// (i.e., `decoder_ffi/aie2_decoder.cpp` relative to this crate's manifest dir).
/// The cc crate resolves the path relative to `CARGO_MANIFEST_DIR`.
