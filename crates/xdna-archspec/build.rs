...
//!
//! Note: `gen_tablegen.rs` (ISA decoder tables) is generated by xdna-emu/build.rs,
//! not here, because the generated code references runtime types that live in
//! xdna-emu's src/tablegen/. The build_helpers/ infrastructure lives here (canonical
//! home) and xdna-emu references it via #[path]. Full migration awaits type refactor.
...
fn main() {
    ...
    // Resolve llvm-aie path: LLVM_AIE_PATH env var or sibling dir.
    ...
    // NOTE: gen_tablegen.rs (ISA decoder tables) is generated by xdna-emu/build.rs,
    // not here. The generated code references runtime types (InstrEncoding, TblgenOutput,
    // etc.) that live in xdna-emu's src/tablegen/ module and cannot be included in
    // archspec until those types are migrated here. xdna-emu/build.rs calls
    // build_helpers::extract::extract_all + generate_tablegen_file using build_helpers/
    // from this crate's directory (via #[path = "crates/xdna-archspec/build_helpers/mod.rs"]).
    // The isa/decoder_tables.rs placeholder will be completed in a future type-migration task.
    //
    // The llvm_aie_path and build_helpers module declarations above are retained so that
    // future tasks can complete the migration without re-threading the plumbing.
    let _ = llvm_aie_path; // suppress unused-variable warning until full migration
}
