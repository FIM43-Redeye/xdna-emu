...
pub fn capability_spine() -> Vec<CapabilityDomain> {
    ...
    vec![
        ...
        d("clock_control", "AM025 Module_Clock_Control / Column_Clock_Control / Reset_Control_1 / AIE_Tile_Column_Reset; aie-rt pm/xaie_clock.c",
          ...
          "Three-tier clock gating MODELED: Column_Clock_Control, per-module Module_Clock_Control (compute/memtile/shim_0/shim_1 layouts decoded from regdb), and adaptive-gate infrastructure (idle counters + abort_period; is_adaptive_dma_engaged/is_adaptive_ss_engaged queries). Silicon-accurate boot default: all tiles gated, ungate-via-CDO is the documented path; ungate_all() test helper exercises the real register-write path. Step loop consults gates before DMA/StreamSwitch/Core stepping; gated-tile non-clock-control writes serve-and-warn (dedup per (col,row,offset); XDNA_EMU_WARN_GATED_ACCESS=0 silences). PARTIAL because: (1) tick_adaptive is not wired into the per-cycle step loop, so adaptive idle counters never advance -- the engagement query returns false unconditionally for v1; (2) Reset_Control_1 / AIE_Tile_Column_Reset (partition teardown) not modeled; (3) no power/cycle-count effect modeled. Privilege enforcement explicitly out of scope (no privilege model in the emulator yet). See docs/superpowers/specs/2026-05-24-clock-control-design.md.",
          Modeled { completeness: Partial { missing: "adaptive idle-counter wiring (tick_adaptive not driven from step loop); Reset_Control_1 / AIE_Tile_Column_Reset partition teardown; power/cycle-count effects".to_string() } }, None),
