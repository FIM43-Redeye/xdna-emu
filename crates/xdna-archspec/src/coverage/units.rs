...
pub fn capability_spine() -> Vec<CapabilityDomain> {
    ...
    vec![
        ...
        d("clock_control", "AM025 Module_Clock_Control / Column_Clock_Control / Reset_Control_1 / AIE_Tile_Column_Reset; aie-rt pm/xaie_clock.c",
          ...
          "Three-tier clock gating MODELED: Column_Clock_Control, per-module Module_Clock_Control (compute/memtile/shim_0/shim_1 layouts decoded from regdb), and adaptive clock gating (idle counters + abort_period + wake-on-event). Step loop consults all three tiers before DMA / StreamSwitch / Core stepping. Adaptive counters tick once per data-movement cycle, gated by per-module clocks as modeled; column or module re-ungate transitions reset the relevant counter, a transition rule not yet specified by the open toolchain or pinned by physical evidence; Phase-5 tick consults DmaEngine::any_channel_has_pending_work so a queued-but-FSM-Idle channel is treated as not-yet-idle. Wake-on-event paths (cycle-accuracy-mission.md item #8): (Wake 1) register-bus access to the Dma/Lock/DataMemory subsystem wakes the DMA counter, StreamSwitch subsystem wakes the SS counter, via DeviceState::wake_adaptive_for_subsystem invoked from write_register/mask_write_register/dma_write; (Wake 2) stream beat arriving at a slave port sets cycle_active, which the existing Phase-5 SS branch converts to tick_adaptive_ss(active=true); (Wake 3) lock-value changes reduce to Wake 1 cross-tile (control-packet decode into Lock register write) plus the Phase-5 has_pending_work check for same-tile (AcquiringLock implies is_active(), so the gate cannot engage). Toolchain-derived boot default: all non-shim tiles are column-gated while the shim follows its MCC reset; ungate-via-CDO is the documented path above the shim; ungate_all() test helper exercises the real register-write path. Gated-tile non-clock-control writes serve-and-warn (dedup per (col,row,offset); XDNA_EMU_WARN_GATED_ACCESS=0 silences). AIE_Tile_Column_Reset (shim 0xFFF28) partition teardown MODELED: asserting bit 0 tears down every non-shim tile in the column (cores, DMAs, locks, stream switches, adaptive counters) via Array::reset_column, preserving tile memory and exempting the shim row, on the assert edge (aie-rt pm/xaie_reset.c); Reset_Control_1/0xFFF14 (shim-NoC reset) is intentionally a no-op pending the NoC stub. Clock-gating counter-freeze MODELED (\"no clock, no tick\"): a clock-gated module advances neither its free-running timer (core/mem module timers) nor its performance counters. Phase 3e consults the Core module clock (compute core bank) or Memory module clock (compute/memtile mem bank), keeping the shim-PL bank independent of the column clock (the separate shim CTE module gate is not yet represented), before ticking -- so gating a module or column freezes Performance_Counter0..3 and the module timers, and they resume from the held value on re-ungate (clock-gating preserves state; the separate AIE_Tile_Column_Reset path is what clears them). An enabled, unfinished core behind either gate is `WaitingForClock`: externally resumable quiescence, never `Halted` or completion; re-ungating resumes the preserved PC. The AIE2 Core module has no adaptive clock gating to model -- Module_Clock_Control (core, 0x60000) exposes only DMA_Adaptive_Clock_Gate (bit 5), Stream_Switch_Adaptive_Clock_Gate (bit 4), and the plain Core_Module_Clock_Enable (bit 2); the AM025 register DB and aie-rt define no Core idle counter, no Core abort_period register, and no cascade wake event. A core waiting on cascade input stalls at the FSM level (Cascade_Stall_MCD/SCD in Core_Status, already modeled via the cascade queues + core stall) -- it does not clock-gate-and-wake -- so the former \"Wake 4\" cascade-arrival item is a non-feature (hardware has no such mechanism), not deferred work. Privilege enforcement is out of scope (no privilege model in the emulator yet) -- a candidate for later bug-probing, not a modeling gap. See docs/superpowers/specs/2026-05-24-clock-control-design.md.",
          Modeled { completeness: Partial { missing: "shim CTE module clock gate".into() } }, None),
