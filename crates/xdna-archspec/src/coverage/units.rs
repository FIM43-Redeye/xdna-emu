...
pub fn capability_spine() -> Vec<CapabilityDomain> {
    ...
    vec![
        ...
        d("debug_halt", "AM025 Debug_*; aie-rt core/",
          ...
          "Halt + status bits modeled. Synchronous PC-event breakpoints (PC_Event0..3 + Debug_Control2.PC_Event_Halt) ARE wired through the interpreter: a matched breakpoint halts the core BEFORE the trap bundle commits (pre-execute seam in coordinator, G1 silicon-derived 2026-05-18; the trap store never lands). Async halt paths and routing of debug-register reads/writes also modeled. Remaining: single-step / count-step PC trap (Debug_Control0 SINGLE_STEP, SSTEP_EVENT count semantics) -- G2/spec section 5.2 follow-up, not in Phase B Unit 1.",
          Modeled { completeness: Partial { missing: "single-step / count-step PC trap (G2/section 5.2)".into() } }, None),
        ...
        d("interrupt", "aie-rt interrupt/, AM025 (L2 shim_intc_l2 23 reg)",
          ...
          Modeled { completeness: Partial { missing: "L2 23-reg surface exhaustiveness + privilege gating".into() } }, None),
