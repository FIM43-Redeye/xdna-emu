...
pub fn capability_spine() -> Vec<CapabilityDomain> {
    ...
    vec![
        ...
        d("debug_halt", "AM025 Debug_*; aie-rt core/",
          ...
          Modeled { completeness: Partial { missing: "programmable breakpoints + single-step PC trap".into() } }, None),
        ...
        d("control_packets", "AM025 Control_Packet_Handler_Status (0x3FF30/0xB0F30); XRT host protocol",
          ...
          "Control-packet headers, reassembly, register read/write effects, response packets MODELED. NPU host instruction stream (WRITE32/BLOCKWRITE/BLOCKSET/MASKWRITE/MASKPOLL/CONFIG_SHIMDMA_*/DDR_PATCH) MODELED. First/Second-header parity and Tlast_Error sticky bits have detecting paths + write-1-to-clear. SLVERR_On_Access deferred: no faithful trigger until the runtime register-access path is repaired (successor plan). Keystone subsystem; was absent from the retired architecture index.",
          Modeled { completeness: Partial { missing: "SLVERR_On_Access sticky bit -- needs a register-access-error model (successor plan)".into() } }, None),
