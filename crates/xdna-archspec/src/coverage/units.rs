...
pub fn capability_spine() -> Vec<CapabilityDomain> {
    ...
    vec![
        ...
        d("interrupt", "aie-rt interrupt/, AM025 (shim interrupt 23 reg: 18 L1 + 5 L2)",
          ...
          "Full AIE2 shim interrupt path MODELED. All 23 registers (18 L1 over 2 switches 0x35000-0x35050, 5 L2 0x15000-0x15010) read- and write-routed with exact semantics (write-1-to-clear status, enable/disable->mask, read-only mask). Stimulus path wired: event/error -> EventModule -> L1 (both switches, per-event-slot independent match + enable gating) -> broadcast network (block-mask honored) -> L2 sink -> NPI line, driven to a fixed point so an L1 output reaches L2 within one dispatch. Hardware errors enter via raise_instr_error -> EventModule. Privilege gating (only the L2 interrupt-routing register) is a driver-side concern, scoped out unrestricted per the noc/shim_mux precedent. Host-visible delivery is firmware-mediated (the AIE interrupt never reaches the x86 host directly) -- Tier B Spec 1 (firmware async-event mailbox plumbing + INSTR_ERROR producer) is shipped: cache + per-column rings + push callback + driver-mirror categorization tables + DRM_AMDXDNA_HW_LAST_ASYNC_ERR ioctl wired through the plugin. Per-class detection follow-ups (DMA / parity / ECC / lock / stream) remain tracked. See docs/superpowers/findings/2026-05-19-interrupt-tier-b-firmware-delivery.md and docs/superpowers/specs/2026-05-19-interrupt-tier-b-firmware-mailbox-design.md.",
          Modeled { completeness: Full }, None),
