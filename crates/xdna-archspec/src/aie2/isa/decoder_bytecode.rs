...
mod tests {
    ...
    ///
    /// Requires `load_from_generated()` from xdna-emu's build-time generated
    /// instruction tables. Re-enabled in Task 7 when archspec's build.rs gains
    /// the TableGen extraction block.
    ///
    /// The equivalent test lives in xdna-emu's `tablegen::decoder_bytecode` module.
    #[test]
    #[ignore = "requires load_from_generated (wired in Task 7)"]
    fn test_real_mv_slot_decode() {
        // Test body requires crate::tablegen::load_from_generated() which is
        // not available in archspec until Task 7. See xdna-emu tablegen tests.
    }
    ...
    ///
    /// Re-enabled in Task 7 when archspec's build.rs gains the TableGen block.
    #[test]
    #[ignore = "requires load_from_generated (wired in Task 7)"]
